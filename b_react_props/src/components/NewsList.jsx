...
const NewsList = () => {
  ...
  return (
    <section className="news-list">
      ...
      <div className="liste">
        {/* So macht man das manuell */}
        {/* <NewsItem superArtikel={artikelArray[0]} />
        <NewsItem superArtikel={artikelArray[1]} />
        <NewsItem superArtikel={artikelArray[2]} />
        <NewsItem superArtikel={artikelArray[3]} /> */}

        {/* forEach würde hier nicht gehen, da es kein array return-> uindefinde! map() gibt uns ein array zurück darum geht das */}
        {artikelArray.map((item, superIndex) => (
          <NewsItem key={superIndex} superArtikel={item} />
        ))}
      </div>
    </section>
  );
